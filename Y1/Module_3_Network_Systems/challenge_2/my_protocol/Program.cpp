...
std::string groupToken = "Don't post your tokens on the internet people ;))))";
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 2){ // possible file number entered
        ...
        if( temp_file.compare("1") == 0 || temp_file.compare("2") == 0 || temp_file.compare("3") == 0
            ...
            std::cout << "Error: The entered argument is not a valid file number, using hardcoded value (" << file << ")!" << std::endl;
        }
    }
    ...
    if (drdtclient.isSimulationFinished()) {
        ...
        framework::NetworkLayer* networkLayer = new framework::NetworkLayer(&drdtclient);
