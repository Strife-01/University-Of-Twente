...
SELECT DISTINCT teacher_id, teacher, 
    ...
    MAX(make_date(year, quarter * 3, 1))::date 
FROM education 
...
-- 7.5
-- a)
SELECT DISTINCT student.student 
FROM student 
WHERE student.study = 'BIT';
-- b)
SELECT DISTINCT course.course 
FROM course_edition, course 
WHERE course_edition.course_code = course.course_code 
AND course_edition.main_teacher = (
  SELECT teacher_id 
  FROM teacher 
  WHERE teacher ILIKE '%Keulen%'
);
-- c)
...
WHERE teacher_id IN (
  SELECT main_teacher
  FROM course_edition, course
  WHERE course.course_code = course_edition.course_code
  AND course.description ILIKE '%databases%'
);
